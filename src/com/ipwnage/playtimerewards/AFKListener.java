				...
				if (data.getPlayerLocation(username).equals(location)) {
					if ((System.currentTimeMillis() / 1000L) - data.getPlayerTimestamp(username) > cm.timeout) {
						if(!data.isAFK(username)) {
							player.sendMessage(String.format(ChatColor.DARK_GREEN + "[Rewards] " + ChatColor.AQUA + "You are now AFK! You are " + ChatColor.RED + "NOT " + ChatColor.AQUA + "receiving money for playing."));
						}
					...
					if(data.isAFK(username)) {
						player.sendMessage(String.format(ChatColor.DARK_GREEN + "[Rewards] " + ChatColor.AQUA + "You are no longer AFK! You are once again receiving money for playing."));
					}
