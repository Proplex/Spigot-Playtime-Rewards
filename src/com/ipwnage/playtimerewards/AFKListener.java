                ...
                if(p.hasPermission("ipwnage.rate.donator")){
                    ...
                    final int tid = cashMoney.getServer().getScheduler().scheduleSyncRepeatingTask(cashMoney, new Runnable() {
                        public void run() {
                            if(cashMoney.logConsole){
                                cashMoney.log.info(String.format("[%s] %s just receiving payment of: %f",cashMoney.getName(), p.getName(), cashMoney.donatorRate));
                            }
                            //Yes. I know this method is deprecated, but it works.
                            if(cashMoney.measeagePlayer){
                                p.sendMessage(net.md_5.bungee.api.ChatColor.DARK_GREEN + String.format("[iPwnAge Rewards] You just receiving %f for playing on the server! Thanks!", cashMoney.donatorRate));
                            }
                    ...
                    final int tid = cashMoney.getServer().getScheduler().scheduleSyncRepeatingTask(cashMoney, new Runnable() {
                        ...
                        public void run() {
                            ...
                            if(cashMoney.measeagePlayer){
                                p.sendMessage(net.md_5.bungee.api.ChatColor.DARK_GREEN + String.format("[iPwnAge Rewards] You just receiving %f for playing on the server! Thanks!", cashMoney.rate));
                            }
                            //Yes. I know this method is deprecated, but it works.
